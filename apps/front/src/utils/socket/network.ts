...
import starx, { type starxMsg } from './wsclient.js'
...
import { Netcfg, SENDTYPE } from './NetworkCfg.js'
...
export class Network {
  ...
  public connect(connectFuc?: Function) {
    ...
    //It is an abnormal disconnection and reconnection, clearing loading and clearing packet sending records
    // if (!this.wsdisconnect) {
    //   console.log('abnormal disconnection and reconnection')
    //   this.sendCount = {}
    // }
  ...
  private connectConnector(event_type: string, event: MessageEvent | CloseEvent | Event | any) {
    switch (event_type) {
      ...
      case 'onKick':
        ...
        if (event.value === 307) {
          const { dispatchSignout } = useUserStore()
          dispatchSignout()
