      ...
      <div class="bg-wrap">
        ...
        <div class="cont">
          ...
          <div class="scoll-num amount sxm7p3g">
            <span v-if="prize.currency == 'BRL'">R$&nbsp;</span
            >{{ animatedNumber.toFixed(2).replace('.', ',') }}
            <span v-if="prize.currency != 'BRL'">{{ prize.currency }}</span>
          </div>
...
.ui-button.s-conic:hover {
  background-image: conic-gradient(from 1turn, rgba(88, 175, 16, 0.7), rgba(29, 128, 58, 0.7));
}
.pfrl3ez > * {
  position: absolute;
